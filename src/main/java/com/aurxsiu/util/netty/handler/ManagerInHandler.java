 ...
 * 如果toMsg有内容,就回复
 * 假设收到的byteBuf中没有-1的Code,那么不会调用{@link ActionWithContent},所以在自动响应的时候,比如{@link DelayExample#main(String[])}中,客户端收到服务端关于delay功能而自动产生的的响应,就不会调用ActionWithContent
 ...
 * 当新建连接,调用内部的ActionWithChannelActive属性,默认什么都不干,通过在构造时自定义ActionWithChannelActive{@link #ManagerInHandler(ActionWithContent, ActionWithChannelActive)}  ManagerInHandler}
 * 主要用于server端获取连接对象发送消息.
 */
public class ManagerInHandler extends ChannelInboundHandlerAdapter {
    ...
    public static interface ActionWithChannelActive {
        public void act(Channel channel);
    }

    private final ActionWithContent messageAction;

    private final ActionWithChannelActive activeAction;
    ...
    public ManagerInHandler(ActionWithContent messageAction, ActionWithChannelActive activeAction) {
        this.messageAction = messageAction;
